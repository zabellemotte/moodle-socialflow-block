...
class block_socialflow extends block_base {
 ...
 function get_required_javascript() {
  //script to manage block with form block options 
...
function get_content() {
       ...
       if( isset( $_POST['socialflow_optionchoice'] ) ){
        ...
        elseif (!is_null(get_user_preferences('socialflow_optionchoice'))) {                   
           $currentchoice=get_user_preferences('socialflow_optionchoice');
       }
       else $currentchoice=14;
        ...
        elseif (!is_null(get_user_preferences('socialflow_typechoice'))) {           
            $currenttype=get_user_preferences('socialflow_typechoice');
        }
        else $currenttype='both';
       ...
       // get items number choice
        ...
        elseif (!is_null(get_user_preferences('socialflow_itemnumchoice'))) {           
            $currentitemnum=get_user_preferences('socialflow_itemnumchoice');
        }
        else $currentitemnum='10';
       ...
       elseif ( !is_null( get_user_preferences('socialflow_courseschoice') ) ) 
       {
           ...
           foreach ($allcurrentcourses as $course_id) {
               $course = $DB->get_record('course', array('id' => $course_id), 'id, visible');
               if ($course && $course->visible) {
               // Add the course to the list of visible courses
                $currentcourses[] = $course_id;
               }
           }
           if($currentcourses==[]){
               $this->content = new stdClass;
       ...
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_optionselectopener','id'=>'socialflow_optionselectopener'));
       ...
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_courseselectopener','id'=>'socialflow_courseselectopener'));
       $this->content->text .=get_string('csotext','block_socialflow');
       $this->content->text .="<span class='expanded-icon'><i class='icon fa fa-chevron-down fa-fw'></i></span>";
       $this->content->text .= html_writer::end_tag('div');

       // button to open type selection form 
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_typeselectopener','id'=>'socialflow_typeselectopener'));
       $this->content->text .=get_string('tsotext','block_socialflow');
       $this->content->text .="<span class='expanded-icon'><i class='icon fa fa-chevron-down fa-fw'></i></span>";
       $this->content->text .= html_writer::end_tag('div');

       // button to open itemnum selection form 
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_itemnumselectopener','id'=>'socialflow_itemnumselectopener'));
       $this->content->text .=get_string('insotext','block_socialflow');
       $this->content->text .="<span class='expanded-icon'><i class='icon fa fa-chevron-down fa-fw'></i></span>";
       $this->content->text .= html_writer::end_tag('div');

       // button to open helptext 
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_helpopener','id'=>'socialflow_helpopener'));
       $this->content->text .=get_string('helpbuttontext','block_socialflow');
       $this->content->text .="<span class='expanded-icon'><i class='icon fa fa-chevron-down fa-fw'></i></span>";
       $this->content->text .= html_writer::end_tag('div');
       
       // OPTION SELECTION FORM
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_optionselectblock','id'=>'socialflow_optionselectblock'));
       $this->content->text .= html_writer::start_tag('form', array('action' => '','method' => 'post','class' => 'socialflow_optionselectform'));
       ...
       // submit button for option selection form
        $this->content->text .=html_writer::start_tag('button',array('class'=>'socialflow_optionselectbutton','onclick'=>'document.getElementById("socialflow_optionselectblock").submit()'));
       ...
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_courseselectblock', 'id'=>'socialflow_courseselectblock'));
       $this->content->text .= html_writer::start_tag('form', array('action' => '','method' => 'post','class' => 'socialflow_courseselectform'));
       ...
       foreach ($courses as $course) {
        ...
        $this->content->text .=html_writer::start_tag('button',array('class'=>'socialflow_courseselectbutton','onclick'=>'document.getElementById("socialflow_courseselectblock").submit()'));
        $this->content->text .=get_string("save","block_socialflow");
        $this->content->text .=html_writer::end_tag('button');
       ...
       // TYPE SELECTION FORM
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_typeselectblock','id'=>'socialflow_typeselectblock'));
       $this->content->text .= html_writer::start_tag('form', array('action' => '','method' => 'post','class' => 'socialflow_typeselectform'));
       // radio boxes with list of options
       ...
       // submit button for type selection form
        $this->content->text .=html_writer::start_tag('button',array('class'=>'socialflow_typeselectbutton','onclick'=>'document.getElementById("socialflow_typeselectblock").submit()'));
        $this->content->text .=get_string("save","block_socialflow");
        $this->content->text .=html_writer::end_tag('button');  
       $this->content->text .= html_writer::end_tag('form');
       $this->content->text .= html_writer::end_tag('div');
      ...
      // ITEM NUMBER SELECTION FORM
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_itemnumselectblock','id'=>'socialflow_itemnumselectblock'));
       $this->content->text .= html_writer::start_tag('form', array('action' => '','method' => 'post','class' => 'socialflow_itemnumselectform'));
       // radio boxes with list of options
       ...
       // submit button for item number selection form
        $this->content->text .=html_writer::start_tag('button',array('class'=>'socialflow_itemnumselectbutton','onclick'=>'document.getElementById("socialflow_itemnumselectblock").submit()'));
        $this->content->text .=get_string("save","block_socialflow");
        $this->content->text .=html_writer::end_tag('button');  
       $this->content->text .= html_writer::end_tag('form');
       $this->content->text .= html_writer::end_tag('div');
        ...
        // HELP BLOCK
       $this->content->text .=html_writer::start_tag('div', array('class'=>'socialflow_helpblock', 'id'=>'socialflow_helpblock'));
       $this->content->text .=get_string('helptext','block_socialflow');
       $surveylink=get_string('surveylink','block_socialflow');
       if ($surveylink!=""){
           $this->content->text .=get_string('surveytextintro','block_socialflow')."<a href='".$surveylink."' target='_blank'> ".get_string('surveytextlink','block_socialflow')."</a>".get_string('surveytextend','block_socialflow');
       }
       $this->content->text .= html_writer::start_tag('form', array('action' => '','method' => 'post','class' => 'socialflow_helpform'));
        // close button for socialflow_helpform
        $this->content->text .=html_writer::start_tag('button',array('class'=>'socialflow_closehelpbutton','onclick'=>'document.getElementById("socialflow_helpblock").style.display = "none";'));
       ...
       // ********************************
        ...
        // The imbricated request with first part that selects the pertinent hits line enables to ensure 
        // good performance (WITH event_hits AS (...)).
        // Closed activities are excluded from social flow when 48 hours passed after closing date 
        // to avoid student despondency (c.closingdate<(".$now."+172800)).
        ...
        // Actions associated to activities or ressources in an hidden section are hidden from the social flow 
        // (cs.visible=1).
        ...
        $sql = "WITH event_hits AS (
                     ...
                     WHERE h.courseid IN (" . $currentcoursesstring . ") AND h.lasttime > " . $loglifetime . " AND c.closingdate > (" . $now . " + 172800)
                 )
                 SELECT ei.id AS hitid, ei.contextid, ei.eventid, ei.courseid, evts.actiontype, evts.moduletable, evts.hasclosingdate, evts.haslatesubmit, evts.latedatefield, c.instanceid, cm.instance, m.name, ei.userids,
                 CASE ";
       ...
       $result = $DB->get_recordset_sql($sql); // https://moodle.org/mod/forum/discuss.php?d=60818
       ...
       if (!$result) {
            ...
            foreach ($result as $row) {
                ...
                $moduleicon = $OUTPUT->pix_icon('icon', $modulenameinlang, $modulename, array('class' => 'iconlarge activityicon'));
                $moduleurl=new moodle_url('/mod/'.$modulename."/view.php", array('id' => $instanceid));
                ...
                // test role of the actual user on course where action occured to know if done/todo informations
                // have to be shown
                ...
                if ($available==1){
                    ...
                    elseif (has_capability('mod/assign:submit', $context,$cuserid, true) ) { // faster than role verification
                        // hits table stores concatenated userids to make access to this information fast 
                        $userids_string=$row->userids;
                        ...
                        if (in_array($cuserid, $userids)){
                           ...
                           $sql52="SELECT lastruntime FROM {task_scheduled} WHERE component LIKE 'logstore_socialflow' AND classname LIKE '%hits%' ";
                           $result52 = $DB->get_record_sql($sql52);
                           if ($result52) {
                               $lastruntimet=$result52->lastruntime;
                               $sql53="WITH recent_log AS (SELECT * FROM {logstore_socialflow_log}  WHERE timecreated>$lastruntimet)
                       ...
                       // Some activities (assign and forum) enable a late date, that does not close the activity, 
                       ...
                       if ($isdone==0) {
                           ...
                           if ($hasclosingdate>0){
                               ...
                               // error on $result6 should not arrise has far as all hits have a closingdate completed, but let make sure it works whatever
                               if ($result6) {
                                   ...
                                   if ($closingdatet==9999999999) {
                                       ...
                                       $closingdate = strftime('%a %d %B %H:%M', $closingdatet);
                                   }
                               }
                               if($haslatesubmit>0){
                                  ...
                                  //but in some additionnal plugins default value is null
                                   ...
                                   if (($result7)&&($result7!==null)) {
                                       ...
                                       $latedate = strftime('%a %d %B %H:%M', $latedatet);
                                       }
                                    }
                               ...
                               if (isset($latedate)){
                                   if ($now<$latedatet){ 
                                       $comment="<br/><span class='socialflow_comment'>".get_string('limitdate','block_socialflow')."<br/>".$latedate."</span>";
                                   } elseif ($now<$closingdatet){
                                       $comment="<br/><span class='socialflow_comment socialflow_critical'>".get_string('latedate','block_socialflow')."<br/>".$closingdate."</span>";
                                   } else {
                                       $comment="<br/><span class='socialflow_comment socialflow_critical'>".get_string('closed','block_socialflow')."</span>";
                                   }
                               } elseif (isset($closingdate)) {
                                  if($now<$closingdatet) {
                                      $comment="<br/><span class='socialflow_comment'>".get_string('limitdate','block_socialflow')."<br/>".$closingdate."</span>";
                                   } else {
                                       $comment="<br/><span class='socialflow_comment socialflow_critical'>".get_string('closed','block_socialflow')."</span>";
                                   }
                               } else $comment="";                     
               ...
               else { // $available=0 and student may not acces to the activity or ressource due to restriction
                   $done='<span class="socialflow_restricted">'.get_string('restricted','block_socialflow').'</span>';
               }
               if ($available==1){
                   $this->content->text.="<tr><td>$shortname</td><td>$moduleicon</td><td>$modulenameinlang</td> <td><a href='".$moduleurl."'>$title</a></td><td>$actiontypelang </td> <td>$freqp%<td> $done </td></tr>";
               } else 
               ...
               $this->content->text.="<tr class='no_access'><td>$shortname</td><td>$moduleicon</td><td>$modulenameinlang</td> <td>$title</td><td>$actiontypelang</td><td>$freqp%<td> $done </td></tr>";
            }
        ...
        if ($result8) {
            ...
            $nextruntime = strftime('%A %d %B %Y %H:%M', $nextruntimet);
            $this->content->text.="<div class='block_socialflow_update'>".get_string('nextupdate','block_socialflow').$nextruntime."</div>";
        }
        return $this->content;
    }
}
